...
graph TB
    ...
    subgraph C1["① USUÁRIOS — acima da linha de interação"]
        ...
        T3("Track 3 · Usuário excluído digital · PID"):::usuario
        T4("Track 4 · Usuário surdo · Balcão Visual"):::usuario
    end

    subgraph C2["② FRONTSTAGE — acima da linha de visibilidade"]
        CEL("Celeste · WhatsApp"):::frontstage
        SVR("Servidor do Balcão · Zoom"):::frontstage
        INT("Intérprete de Libras"):::frontstage
        SPID("Servidor do PID"):::frontstage
        ATER("Servidor de Atermação"):::frontstage
    end

    subgraph C3["③ BACKSTAGE — acima da linha de interação interna"]
        GEST("Gestores das Varas / Secretarias"):::backstage
        TI("Equipe de TI e Infraestrutura"):::backstage
        OUV("Ouvidoria"):::backstage
        MAG("Magistrados"):::backstage
    end

    subgraph C4["④ SISTEMAS DE SUPORTE"]
        PJE("PJe / eProc"):::sistema
        ZOOM("Zoom Platform"):::sistema
        CELM("Motor Celeste + Logs"):::sistema
        DJ("DataJud"):::sistema
    end

    subgraph C5["⑤ GOVERNANÇA E REGULAÇÃO"]
        CNJ("CNJ"):::gov
        CSJT("CSJT"):::gov
        ANPD("ANPD · LGPD"):::gov
        PROV("Zoom Inc."):::gov
    ...
    %% Interações usuário → frontstage
    T1A & T1B & T2  -->|triagem|        CEL
    T3              -->|mediado por|    SPID
    T4              -->|mediado por|    INT
    CEL             -->|handoff Zoom|   SVR
    SPID & INT      -->|acessa|         SVR
    T1A & T1B & T3  -->|encaminha para| ATER
    SVR             -->|consulta|       PJE

    %% Backstage → frontstage
    GEST -->|escala e protocolo|   SVR
    MAG  -->|escopo informacional| SVR
    TI   -->|mantém|               CELM
    TI   -->|mantém|               ZOOM
    OUV  -->|monitora via|         DJ

    %% Sistemas e governança (tracejado)
    CEL  -. roda em .->    CELM
    SVR  -. roda em .->    ZOOM
    DJ   -. auditado por .-> CNJ
    CSJT -. governa TI .-> TI
...
| Usuário excluído digital | ① Usuário | Externo | 3 | Todas (mediadas) | Acessa o serviço via PID com auxílio de servidor | Barreira de agendamento com SGJ; dependência de deslocamento físico |
...
| **Servidor do Balcão** | ② Frontstage | Interno | Todos | Atend., Pós | Atendimento síncrono via Zoom; consulta PJe; orienta encaminhamentos | **H2 Opacidade da Fila** · **Fadiga do Zoom** (Bailenson, 2021) |
...
| **CNJ** | ⑤ Governança | Regulatório | — | — | Obrigatoriedade e métricas do Balcão Virtual | Redesenho deve alinhar indicadores às metas CNJ/DataJud |
| **CSJT** | ⑤ Governança | Regulatório | — | — | Governa TI da Justiça do Trabalho; mandatou migração para Zoom | Mudanças de plataforma requerem aprovação via PGTIC-JT (Res. 425/2025) |
